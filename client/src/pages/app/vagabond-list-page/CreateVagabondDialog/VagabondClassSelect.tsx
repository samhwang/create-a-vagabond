...
export function VagabondClassSelect<T extends FieldValues>({ queryRef, ...props }: VagabondClassSelectProps<T>) {
    ...
    graphql`
      ...
      @argumentDefinitions(count: { type: "Int", defaultValue: 50 }, cursor: { type: "String" }) {
        vagabondClassConnection(first: $count, after: $cursor)
          @connection(key: "VagabondClassSelect_query_vagabondClassConnection") {
          edges {
            node {
              id
              name
